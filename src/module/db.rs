...
use crate::model::DateOrDateTime;
...
impl DatabaseCheck {
    pub fn new(connection: MySqlPool, _type: i32, mut details: Vec<String>) -> Self {
        ...
        Self { connection, _type, host, port, user, passwd, db_name, tables }
    }
    ...
    async fn mysql(&self, pool: MySqlPool, main_id: String, from: String, table: String, columns: Vec<String>) -> String {
        ...
        let result_client: Vec<Vec<String>> = sqlx::query(&query_client).fetch_all(&pool).await.unwrap()
            .into_iter()
            .map(|row| Self::set_query(use_column.clone(), &row))
            .collect();
    ...
    pub async fn build(&self) -> Result<Vec<String>, sqlx::Error> {
        let mut message = vec![];
        let query = format!(
        ...
        match self._type {
            1 => {
                for i in &self.tables {
                    ...
                    let pool = MySqlPool::connect(&format!("mysql://{}:{}@{}:{}/{}", self.user, self.passwd, self.host, self.port, self.db_name)).await?;
